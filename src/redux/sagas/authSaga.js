...
function* handleError(e) {
	...
	switch (e.code) {
		case 'auth/network-request-failed':
			yield put(setAuthStatus({ ...obj, message: 'Произошла сетевая ошибка. Пожалуйста, попробуйте еще раз.' }));
			break;
		case 'auth/email-already-in-use':
			yield put(setAuthStatus({ ...obj, message: 'Электронная почта уже используется. Пожалуйста, используйте другую почту' }));
			break;
		case 'auth/wrong-password':
			yield put(setAuthStatus({ ...obj, message: 'Неправильная электронная почта или пароль' }));
			break;
		case 'auth/user-not-found':
			yield put(setAuthStatus({ ...obj, message: 'Неправильная электронная почта или пароль' }));
			break;
		case 'auth/reset-password-error':
			yield put(setAuthStatus({ ...obj, message: 'Не удалось отправить пароль сбросить электронную почту. Правильно ли вы набрали электронную почту?' }));
			break;
		default:
